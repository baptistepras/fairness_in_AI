...
def preds_todf(df, dataset, label_decoder, model, preds_col):
    for idx in range(len(dataset.imgs)):
        ...
        preds = pred.max(1)[1].detach().cpu().item()  # Calcul de la prédiction (classe discrète)
        probabilities = torch.nn.functional.softmax(pred, dim=1)
        confidence = probabilities[0, preds].item()  # Calcul de la probabilité (confiance) associée à la classe prédite via softmax
        df.loc[df["Image Index"] == img_name, preds_col] = label_decoder[preds]
        df.loc[df["Image Index"] == img_name, "labels"] = label_decoder[label]
        df.loc[df["Image Index"] == img_name, "confidence"] = confidence
    return df
